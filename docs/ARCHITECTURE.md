...
- 시각 표시는 형제 `.selection-mark`가 담당한다. 체크박스는 `--action` 채움과
  고대비 체크, 라디오는 같은 색 역할의 원형 마크를 사용하고 회전이나 polygon으로
  작은 컨트롤의 판독성을 해치지 않는다.
- 병합 파일·그룹 선택, YouTube 업로드 source, 아동용 설정이 같은 컨트롤 계열을
  공유한다. 완료 화면과 source 소진 상태는 선택색 대신 `--ok-*` 성공색을 사용하며,

...
class TestStreamDownloader:
    ...
    def test_download_success(self, mock_ydl_class, mock_subprocess, downloader):
        ...
        mock_ydl.extract_info.return_value = {'url': 'https://direct.stream.url/video.m3u8'}
    ...
    @patch('src.yt_monitor.downloader.subprocess.run')
    @patch('src.yt_monitor.downloader.yt_dlp.YoutubeDL')
    def test_download_with_custom_prefix(self, mock_ydl_class, mock_subprocess, downloader):
        mock_ydl = MagicMock()
        mock_ydl.__enter__ = Mock(return_value=mock_ydl)
        mock_ydl.__exit__ = Mock(return_value=False)
        mock_ydl.extract_info.return_value = {'url': 'https://direct.stream.url/video.m3u8'}
    ...
    def test_download_with_realtime_split_time_mode(self, mock_get_url, mock_subprocess, temp_dir):
        downloader = StreamDownloader(
            download_directory=temp_dir,
            download_format="best",
            split_mode="time",
    ...
    @patch('src.yt_monitor.downloader.subprocess.run')
    @patch('src.yt_monitor.downloader.StreamDownloader._get_direct_stream_url')
    def test_download_with_realtime_split_size_mode(self, mock_get_url, mock_subprocess, temp_dir):
        downloader = StreamDownloader(
            download_directory=temp_dir,
            download_format="best",
            split_mode="size",
    ...
    @patch('src.yt_monitor.downloader.subprocess.run')
    @patch('src.yt_monitor.downloader.StreamDownloader._get_direct_stream_url')
    def test_download_with_realtime_split_failure(self, mock_get_url, mock_subprocess, downloader):
        mock_get_url.return_value = 'https://direct.stream.url/video.m3u8'

...
from .channels.models import GlobalSettingsDTO
from .channels.repository import ChannelManager
from .logging import Logger
from .maintenance.cleanup import FileCleaner
...
def cleanup_mode(args):
    ...
    try:
        ...
        print(f"\n보관 기간: {summary['retention_days']}일")
        print(f"삭제 대상 파일: {summary['files_to_delete']}개")
        print(f"삭제 예정 용량: {summary['total_size_mb']:.2f} MB")
        print(f"보존되는 라이브 파일: {summary['live_files_preserved']}개")
        print(f"라이브 폴더 용량: {summary['live_size_mb']:.2f} MB")
        print()
        ...
        if args.dry_run:
            print("[DRY RUN] 실제 삭제 없이 목록만 표시합니다.\n")

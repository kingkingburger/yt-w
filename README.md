...
- 영상 관리 탭에서 서버의 모든 영상을 폴더별로 보고 검색·전체 선택·선택 삭제. YouTube 업로드 탭에서도 같은 방식으로 파일 삭제 가능
...
- 여러 영상을 체크하면 한 번에 작업 등록. 제목은 파일마다 파일명에서 자동 생성(같은 날 녹화처럼 겹치면 녹화 시각을 남겨 구분), 설명·태그·카테고리는 공통 적용
...
- 서버 영상 목록에는 `업로드 완료`, `미업로드`, `업로드 중`을 표시합니다.
  완료 기록은 다운로드 루트의 `.youtube-upload-history.sqlite3`에 저장하여 재시작
  후에도 유지합니다. 경로·파일 크기·수정 시각이 같은 영상만 완료로 표시합니다.
  이 기능 적용 전 사라진 기록이나 다른 곳에서 올린 영상은 판별하지 못하며,
  YouTube에서 영상을 삭제해도 이 앱의 완료 기록은 남습니다.
